...
This update introduces critical bug fixes related to compilation
...
Refactored validation logic for `pre_compile`, `post_compile`, `pre_install`, and `post_install` properties to support
...
This update introduces a minor bug fix to the APCU cache layer
...
### Fixed
 - Removed deprecated used of `curl_close`



## [3.1.0] - 2026-05-26

This update introduces support for php 8.5, making the minimum required version becoming php 8.2
...
### Fixed
 - Fixed issue where ncc would fail to install packages from remote repositories such as Github when the rate limit
   is exceeded, this should usually fall back to using `git` to get around the rate limit but the resolution of the
   git repository uses the API. This is now fixed by assuming the git repository URL if all else fails
...
This update introduces critical bug fixes and changes

### Added
 - Added new global option `--simplified`, `-s` which enables simplified output mode for commands that support it,
   this mode reduces the amount of output generated by ncc to only essential information, useful for scripting and
   automation or integration with other tools.
 - Added a new project & build configuration property called 'extensions' which defines the required PHP extensions
   needed for the project to run, during installation ncc will check if the extensions are installed and enabled. This
   check can be skipped by using the `--skip-extensions` option when installing the project.

### Changed
 - Updated dependency `fslib`
 - Updated dependency `Process`
 - Updated dependency `deprecation-contracts`
 - Updated Template IO calls to use builtin `file_get_contents` for compatibility with Phar builds

### Fixed
 - Fixed various IO related issues with file operations
